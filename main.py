...
def create_login_code(session: SessionDep, response: Response) -> dict:
    ...
    session.execute(delete(LoginToken).where(
        LoginToken.created_at < datetime.now() - timedelta(seconds=LOGIN_TOKEN_TTL_SECONDS),
    ))
...
def confirm_login_code(
    ...
    result = session.execute(update(LoginToken).where(
    ...
    if result.rowcount == 0:
        ...
        raise HTTPException(status_code=409, detail='Код уже підтверджено іншим користувачем')
...
def poll_login_code(token: str, response: Response, session: SessionDep) -> dict:
    ...
    user_id = session.execute(delete(LoginToken).where(
        LoginToken.token == token,
        LoginToken.user_id.is_not(None),
        LoginToken.created_at >= cutoff,
    ).returning(LoginToken.user_id)).scalar_one_or_none()
    session.commit()
    response.headers['Cache-Control'] = 'no-store'
...
def export_history(user: CurrentUser, session: SessionDep) -> JSONResponse:
    ...
    session.refresh(user)
    habits = session.exec(select(Habit).where(Habit.user_id == user.id).order_by(Habit.id)).all()
    checkins = session.exec(
        select(Checkin).join(Habit).where(Habit.user_id == user.id).order_by(Checkin.day, Checkin.id)
    ).all()
    return JSONResponse({
        'version': 1,
        'exported_at': now_utc().isoformat(),
        'user': UserPublic.model_validate(user).model_dump(mode='json'),
        'habits': [HabitPublic.model_validate(habit).model_dump(mode='json') for habit in habits],
...
def reminder_targets(_: RequireBot, session: SessionDep) -> list[dict]:
    users = session.exec(select(User).where(
        User.telegram_id.is_not(None), User.reminders_enabled.is_(True),
    )).all()
    return [{
...
def reminder_sent(telegram_id: int, data: ReminderSent, _: RequireBot, session: SessionDep) -> dict:
...
@app.get("/habits")
def list_habits(user: CurrentUser, session: SessionDep, include_archived: bool = False) -> list[HabitPublic]:
...
def create_habit(
    ...
    if start_date > user_today(user):
        raise HTTPException(status_code=422, detail='Дата початку не може бути в майбутньому')
    habit = Habit.model_validate(data, update={"user_id": user.id, 'start_date': start_date})
    session.add(habit)
    session.commit()
...
def update_habit(
    ...
    if 'archived' in changes:
        ...
        habit.archived_at = (habit.archived_at or user_today(user)) if archived else None
...
def list_checkins(
    ...
    if since is not None and until is not None and since > until:
        raise HTTPException(status_code=422, detail='Початок періоду має бути не пізніше кінця')
...
def create_checkin(
    ...
    try:
        ...
        session.execute(update(Habit).where(
            Habit.id == habit_id,
...
@app.get("/stats")
def all_stats(user: CurrentUser, session: SessionDep, include_archived: bool = False) -> list[HabitStats]:
    ...
    return [
        HabitStats(
            habit_id=habit.id,
            **stats.compute(days_by_habit.get(habit.id, []), today, weekdays=habit.weekdays,
                            start_date=habit.start_date, archived_at=habit.archived_at),
        )
        for habit in habits
    ]
...
def habit_stats(habit_id: int, user: CurrentUser, session: SessionDep) -> HabitStats:
    ...
    return HabitStats(habit_id=habit_id, **stats.compute(
        list(days), user_today(user), weekdays=habit.weekdays,
        start_date=habit.start_date, archived_at=habit.archived_at,
    ))
