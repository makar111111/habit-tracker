  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: string, options: RequestInit = {}) => {
      ...
      if (path === "/auth/login-code")
        return respond({
          token: "login-token",
          url: "https://t.me/example?start=login-token",
          expires_in: 60,
        });
      if (path === "/auth/login-code/login-token") {
        signedIn = true;
        return respond({ status: "confirmed" });
      }
      ...
      if (path === "/habits") {
        ...
        return respond(
          habits.filter((habit) => url.searchParams.has("include_archived") || !habit.archived_at),
        );
      }
      if (path === "/stats")
        return respond(
          habits
            .filter((habit) => url.searchParams.has("include_archived") || !habit.archived_at)
            .map((habit) => ({
              habit_id: habit.id,
              total: habit.archived_at
                ? 200
                : checkins.filter((c) => c.habit_id === habit.id).length,
              current_streak: 0,
              longest_streak: 4,
              done_today: checkins.some((c) => c.habit_id === habit.id && c.day === today),
              last_day: null,
            })),
        );
      ...
      if (historyPath) {
        ...
        return respond(
          checkins.filter(
            (c) =>
              c.habit_id === id &&
              c.day >= (url.searchParams.get("since") ?? "") &&
              c.day <= (url.searchParams.get("until") ?? "9999"),
          ),
        );
      }
...
describe("Сеанс і відновлення після помилок", () => {
  ...
  it("запізніле збереження профілю після виходу не відновлює завершений сеанс", async () => {
    ...
    const unsubscribe = client.getQueryCache().subscribe((event) => {
      if (event.type === "updated" && event.query.queryKey[0] === "me")
        laterProfiles.push(event.query.state.data);
    });
    await act(async () => {
      finishSave();
    });
  ...
  it("підтвердження Telegram відкриває звички", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Відкрити бота" })).toHaveAttribute(
      "href",
      "https://t.me/example?start=login-token",
    );
...
describe("Розклад і профіль", () => {
  it("сьогодні та прогрес враховують дату користувача і лише заплановані звички", async () => {
    ...
    await waitFor(() =>
      expect(
        calls.find((call) => call.method === "POST" && call.path === "/habits/1/checkins")?.body,
      ).toEqual({ day: today }),
    );
  });
  ...
  it("створює звичку з датою початку та вибраними днями", async () => {
    ...
    for (const label of ["Вт", "Ср", "Чт", "Пт", "Сб", "Нд"])
      await userEvent.click(screen.getByRole("checkbox", { name: label }));
    await userEvent.click(screen.getByRole("button", { name: "Додати" }));
    await waitFor(() =>
      expect(
        calls.find((call) => call.path === "/habits" && call.method === "POST")?.body,
      ).toMatchObject({ name: "Біг", start_date: "2026-09-12", weekdays: [0] }),
    );
  ...
  it("зберігає налаштування нагадувань і пропонує експорт", async () => {
    ...
    expect(
      calls.find((call) => call.path === "/users/me" && call.method === "PATCH")?.body,
    ).toEqual({
      name: "Марія",
    ...
    expect(screen.getByRole("link", { name: "Завантажити мої дані (JSON)" })).toHaveAttribute(
      "href",
      "/users/me/export",
    );
  });
...
describe("Історія, архів та аналітика", () => {
  ...
  it("помилка календаря має повторення, а після нього можна додати й зняти минулий день", async () => {
    ...
    expect(
      calls.some((call) => call.method === "DELETE" && call.path.endsWith("/checkins/2026-09-11")),
    ).toBe(true);
    ...
    await waitFor(() =>
      expect(
        calls.some(
          (call) =>
            call.path.includes("since=2026-02-22") && call.path.includes("until=2026-06-21"),
        ),
      ).toBe(true),
    );
  });
