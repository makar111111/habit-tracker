...
afterEach(() => { cleanup(); vi.useRealTimers(); vi.restoreAllMocks(); focusManager.setFocused(undefined); });
...
it("оновлює дату на найближчій межі хвилини й перераховує статистику після опівночі", async () => {
  ...
  const { result } = renderHook(() => { useStats(true); return useToday(true); }, { wrapper });
