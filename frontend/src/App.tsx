...
const AnalyticsScreen = lazy(() =>
  import("./analytics/AnalyticsScreen").then((module) => ({ default: module.AnalyticsScreen })),
);
const TABS = [
...
export function App() {
  ...
  const me = useMe();
  useEffect(() => {
    if (me.data === null) clearPrivateQueries(client);
  }, [client, me.data]);
  if (me.isPending)
    return (
      <main className="shell" aria-busy="true">
        <p className="chart-note">Завантажую профіль…</p>
      </main>
    );
  if (me.isError && !isUnauthorized(me.error))
    return (
      <main className="shell">
        <QueryError error={me.error} retry={me.refetch} busy={me.isFetching} />
      </main>
    );
    ...
    <main className="shell">
      ...
      <div role="tabpanel" id={`panel-${tab}`} aria-labelledby={`tab-${tab}`} tabIndex={0}>
        {tab === "settings" ? (
          ...
          <QueryError error={day.error} retry={day.refetch} busy={day.isFetching} />
        ) : !today ? (
          <div className="skeleton" aria-label="Завантажую дату" />
        ) : tab === "today" ? (
          <TodayScreen today={today} />
        ) : tab === "archive" ? (
          <ArchiveScreen today={today} />
        ) : (
          <Suspense fallback={<div className="skeleton" aria-label="Завантажую аналітику" />}>
