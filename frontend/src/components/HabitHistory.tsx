    ...
    <div className="habit-history">
      <div className="calendar-navigation">
        <button
          ...
          disabled={pending}
          aria-label="Попередні 12 тижнів"
        >
          ← Раніше
        </button>
        <span>
          {toISO(gridStart(end))} — {toISO(end)}
        </span>
        <button
          type="button"
          className="ghost-button"
          ...
          aria-label="Наступні 12 тижнів"
        >
          Пізніше →
        </button>
      </div>
      {query.isError ? (
        <QueryError error={query.error} retry={query.refetch} busy={query.isFetching} />
      ) : query.isLoading ? (
        <p className="chart-note" aria-busy="true">
          Завантажую відмітки…
        </p>
      ) : (
        <Calendar
          ...
          disabled={pending}
          onToggle={(day, done) => {
            if (!pending) toggle.mutate({ habitId: habit.id, day, done, today: toISO(today) });
          }}
        />
      )}
    </div>
