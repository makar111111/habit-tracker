...
describe("NewHabit", () => {
  it("коли звички є, згорнута в одну кнопку", () => {
    ...
    expect(screen.getByRole("button", { name: "Нова звичка" })).toHaveAttribute("aria-expanded", "false");
  ...
  it("розгортається й одразу ставить фокус на назву", async () => {
    ...
    expect(screen.getByRole("button", { name: "Нова звичка" })).toHaveAttribute("aria-expanded", "true");
