...
export function AddHabitForm({ today, id, autoFocus, onCreated, onCancel }: Props) {
  ...
  function submit(event: React.FormEvent) {
    ...
    create.mutate({ name: name.trim(), description: description.trim(), start_date: startDate, weekdays }, {
      onSuccess: () => { setName(""); setDescription(""); setSelectedDate(null); setWeekdays(EVERY_DAY); onCreated?.(); },
    });
  }

  return <form className="add-form" id={id} onSubmit={submit}
    onKeyDown={(event) => { if (event.key === "Escape" && onCancel) { event.preventDefault(); onCancel(); } }}>
    <div className="add-form-main">
      <input value={name} onChange={(event) => setName(event.target.value)} placeholder="Назва звички" maxLength={100} required aria-label="Назва звички" autoFocus={autoFocus} />
    ...
    <div className="add-form-actions">
      ...
      {onCancel && <button type="button" className="ghost-button" onClick={onCancel}>Скасувати</button>}
    </div>
  </form>;
}
