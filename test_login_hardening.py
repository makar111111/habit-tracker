...
def test_confirmation_cannot_change_the_owner(login):
    ...
    assert login.post('/auth/confirm', json={'token': token}, headers=headers(111)).status_code == 200
    assert login.post('/auth/confirm', json={'token': token}, headers=headers(111)).status_code == 200
    assert login.post('/auth/confirm', json={'token': token}, headers=headers(222)).status_code == 409
...
def test_issuing_code_removes_expired_rows(login, session):
    session.add(LoginToken(token='expired', created_at=datetime.now() - timedelta(days=1)))
...
def test_parallel_exchange_issues_only_one_session(login, tmp_path):
    engine = create_engine(f"sqlite:///{tmp_path / 'login.db'}", connect_args={'check_same_thread': False})
    ...
    try:
        ...
        assert sum(auth.SESSION_COOKIE in response.cookies for response in responses) == 1
    finally:
