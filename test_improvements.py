...
def test_trim_names_and_reject_blank_create(client):
    ...
    assert (
        client.post("/habits", json={"name": "  Reading  "}).json()["name"] == "Reading"
    )
...
def test_non_ascii_bot_secret_is_unauthorized(client, bot_headers, path):
    bot_headers(123)
    unchecked = TestClient(main.app, raise_server_exceptions=False)
    result = unchecked.get(
        path, headers=[(b"x-telegram-id", b"123"), (b"x-bot-secret", b"\xe9")]
    )
...
@pytest.mark.parametrize(
    "telegram_id",
    ["0", "-1", "not-a-number", str(2**63), "9" * 5000],
    ids=["zero", "negative", "non-numeric", "overflow", "very-long"],
)
def test_invalid_telegram_id_is_unauthorized_without_creating_user(
    client, bot_headers, session, telegram_id
):
...
def test_invalid_weekday_schedule_is_rejected(client, weekdays):
    assert (
        client.post(
            "/habits", json={"name": "Training", "weekdays": weekdays}
        ).status_code
        == 422
    )
...
def test_archive_preserves_history_and_restore(client, habit_id):
    ...
    assert (
        client.patch(f"/habits/{habit_id}", json={"archived": False}).json()[
            "archived_at"
        ]
        is None
    )
...
def test_archive_is_owner_scoped(client, habit_id, bot_headers):
    assert (
        client.patch(
            f"/habits/{habit_id}", json={"archived": True}, headers=bot_headers(111)
        ).status_code
        == 404
    )
...
def test_archive_rejects_later_checkins(client, habit_id):
    ...
    assert (
        client.post(
            f"/habits/{habit_id}/checkins", json={"day": later.isoformat()}
        ).status_code
        == 422
    )
...
def test_backfill_keeps_earliest_start_when_another_request_read_old_state(
    client, habit_id, session
):
    ...
    with Session(session.get_bind()) as first, Session(session.get_bind()) as second:
        ...
        main.create_checkin(
            habit_id, CheckinCreate(day=date(2020, 3, 1)), first_user, first
        )
        main.create_checkin(
            habit_id, CheckinCreate(day=date(2020, 4, 1)), second_user, second
        )
...
def test_user_local_day_controls_default_checkin(client, habit_id, monkeypatch):
    ...
    assert (
        client.post(f"/habits/{habit_id}/checkins", json={}).json()["day"]
        == "2026-09-13"
    )
...
def test_export_includes_archived_history_only_for_owner(client, habit_id, bot_headers):
    ...
    foreign = client.post(
        "/habits", json={"name": "Private"}, headers=bot_headers(111)
    ).json()
    ...
    assert not any(
        key in data["user"] for key in ["last_reminder_day", "token", "secret"]
    )
...
def test_reminder_targets_are_private_and_delivery_is_persisted(client, bot_headers):
    ...
    client.patch(
        "/users/me", json={"reminders_enabled": False}, headers=bot_headers(222)
    )
    ...
    assert (
        client.post(
            "/reminder-targets/111/sent", json={"day": "2026-09-12"}, headers=headers
        ).status_code
        == 200
    )
    assert (
        client.get("/reminder-targets", headers=headers).json()[0]["last_reminder_day"]
        == "2026-09-12"
    )
    client.post(
        "/reminder-targets/111/sent", json={"day": "2026-09-11"}, headers=headers
    )
    assert (
        client.get("/reminder-targets", headers=headers).json()[0]["last_reminder_day"]
        == "2026-09-12"
...
def test_scheduled_streak_survives_rest_days():
    ...
    assert (
        stats.compute(days, date(2026, 9, 15), weekdays=[0, 2, 4])["current_streak"]
        == 0
    )
...
def test_streak_ignores_days_outside_schedule_and_future():
    ...
    result = stats.compute(
        days, date(2026, 9, 12), weekdays=[0, 2, 4], start_date=date(2026, 9, 9)
    )
