...
def test_upgrade_preserves_old_data_and_takes_pre_upgrade_snapshot(old_database):
    ...
    with closing(sqlite3.connect(result.backup_path)) as snapshot:
        ...
        assert "weekdays" not in {
            row[1] for row in snapshot.execute("PRAGMA table_info(habit)")
        }
    ...
    with engine.connect() as connection:
        ...
        assert connection.exec_driver_sql(
            "SELECT id, telegram_id, name FROM user ORDER BY id"
        ).all() == [(41, 700001, "Олена"), (42, None, "Локальний")]
        assert connection.exec_driver_sql(
            "SELECT id, user_id, name, description FROM habit ORDER BY id"
        ).all() == [(101, 41, "Читання", "Щодня"), (102, 42, "Рух", "")]
        assert connection.exec_driver_sql(
            "SELECT timezone, reminder_hour, reminders_enabled, last_reminder_day FROM user"
        ).all() == [("Europe/Kyiv", 20, 1, None), ("Europe/Kyiv", 20, 1, None)]
        habits = connection.exec_driver_sql(
            "SELECT start_date, weekdays, archived_at FROM habit ORDER BY id"
        ).all()
        assert habits[0][0] == "2025-01-01"
        assert habits[1][0] in {before_today, after_today}
        assert all(
            json.loads(row[1]) == list(range(7)) and row[2] is None for row in habits
        )
        assert connection.exec_driver_sql(
            "SELECT * FROM checkin ORDER BY id"
        ).all() == [(301, 101, "2025-01-03"), (302, 101, "2025-01-01")]
        assert connection.exec_driver_sql("SELECT * FROM logintoken").all() == [
            ("test-token", "2026-09-01 10:00:00", 41)
        ]
...
def test_repeat_upgrade_is_noop_and_preserves_user_choices(old_database):
    ...
    with engine.begin() as connection:
        connection.exec_driver_sql(
            "UPDATE user SET timezone='UTC', reminder_hour=9, reminders_enabled=0 WHERE id=41"
        )
        connection.exec_driver_sql(
            "UPDATE habit SET weekdays='[0,2,4]', archived_at='2026-09-01' WHERE id=101"
        )
    ...
    with engine.connect() as connection:
        assert connection.exec_driver_sql(
            "SELECT timezone, reminder_hour, reminders_enabled FROM user WHERE id=41"
        ).one() == ("UTC", 9, 0)
        assert connection.exec_driver_sql(
            "SELECT weekdays, archived_at FROM habit WHERE id=101"
        ).one() == ("[0,2,4]", "2026-09-01")


@pytest.mark.parametrize("file_database", [False, True])
def test_new_database_is_created_at_current_version_without_backup(
    tmp_path, file_database
):
    migrations = importlib.import_module("migrations")
    engine = create_engine(
        f"sqlite:///{(tmp_path / 'new.sqlite3').as_posix()}"
        if file_database
        else "sqlite://"
    )
    try:
        ...
        assert {"user", "habit", "checkin", "logintoken"} <= set(
            inspect(engine).get_table_names()
        )
        assert {"start_date", "weekdays", "archived_at"} <= {
            column["name"] for column in inspect(engine).get_columns("habit")
        }
        assert {
            "timezone",
            "reminder_hour",
...
def test_backup_failure_aborts_before_any_schema_change(old_database, monkeypatch):
    ...
    with engine.connect() as connection:
        ...
        assert (
            connection.exec_driver_sql("SELECT COUNT(*) FROM checkin").scalar_one() == 2
        )
        assert "weekdays" not in {
            column["name"] for column in inspect(connection).get_columns("habit")
        }
...
def test_broken_foreign_keys_abort_upgrade_and_rollback(old_database):
    ...
    with engine.begin() as connection:
        ...
        connection.exec_driver_sql(
            "INSERT INTO checkin VALUES (303, 9999, '2025-01-02')"
        )
...
def test_failed_verification_rolls_back_already_applied_columns(
    ...
    def fail_after_changes(connection, *, current):
        if current:
            ...
            raise migrations.MigrationError(
                "injected post-migration validation failure"
            )
...
def test_unsupported_dialect_is_rejected_without_connecting():
    ...
    engine = create_mock_engine(
        "postgresql://", lambda *args: pytest.fail("must not connect")
    )
