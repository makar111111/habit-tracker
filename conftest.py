...
def client_fixture(session: Session, monkeypatch: pytest.MonkeyPatch):
    ...
    Локальний режим вмикається ЯВНО. У бойовому застосунку він вимкнений
    за замовчуванням (запит без доказів отримує 401), бо на сервері
    означав би «хто дістався до порту — той і господар». А більшість
    тестів у test_api.py перевіряють саме CRUD, і змушувати кожен із них
    проходити вхід через Telegram було б шумом, який ховає суть тесту.

    Явний рядок тут кращий за мовчазне «якось працює»: видно, що тести
    користуються режимом розробки, а не тим, що бачить справжній сервер.
    """
